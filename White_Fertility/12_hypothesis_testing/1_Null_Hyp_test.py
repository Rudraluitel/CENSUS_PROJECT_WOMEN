...
# Null Hypothesis (H₀): The principal components do not explain a significant portion of the variance in white births.
# Alternative Hypothesis (H₁): At least one principal component does explain a significant portion of the variance.
...
if p_value < alpha:
    print(
        "Reject the null hypothesis: The principal components collectively explain a significant portion of the variance in white births."
    )
else:
    print(
        "Fail to reject the null hypothesis: The principal components do not explain a significant portion of the variance in white births."
    )
...
for i, p_val in enumerate(component_p_values, start=1):
    if p_val < alpha:
        print(
            f"Reject the null hypothesis for Principal Component {i}: It significantly explains the variance in white births."
        )
    else:
        print(
            f"Fail to reject the null hypothesis for Principal Component {i}: It does not significantly explain the variance in white births."
        )
