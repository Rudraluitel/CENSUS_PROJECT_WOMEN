...
random_noise = np.random.normal(0, noise_level, len(y_pred))
y_pred_noisy = y_pred * (1 + random_noise)
...
# Add noise to the future predictions
random_noise_future = np.random.normal(0, noise_level, len(y_future_pred))
y_future_pred_noisy = y_future_pred * (1 + random_noise_future)

# Combine actual and predicted data for plotting
all_years = np.append(df["Year"], future_years["Year"])
all_births = np.append(y, y_future_pred_noisy)
...
# Plot predicted white births from 2023 to 2026 with noise
plt.plot(
    all_years,
    all_births,
    color="red",
    linestyle="--",
    marker="x",
    label="Predicted Births with Noise",
)

# Label the chart
plt.xlabel("Year")
plt.ylabel("White Births")
plt.title("Actual vs Predicted White Births (2010-2026) with Noise")
plt.legend()
plt.grid(True)
